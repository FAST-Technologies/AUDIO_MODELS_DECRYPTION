...
from .GigaAM_PyTorch import GigaAM_PyTorch
from .GigaAMASR_PyTorch import GigaAMASR_PyTorch
...
def _download_file_PyTorch(file_url: str,
                   file_path: str
) -> str:
...
@property
def _download_model_PyTorch(model_name: str,
                            download_root: str
) -> Tuple[str, str]:
    ...
    return model_name, _download_file_PyTorch(model_url, model_path)

# Функция для выкачки токенайзера
@property
def _download_tokenizer_PyTorch(model_name: str,
                                download_root: str
) -> Optional[str]:
    ...
    return _download_file_PyTorch(tokenizer_url, tokenizer_path)
...
def load_model_PyTorch(
...
) -> Union[GigaAM_PyTorch, GigaAMASR_PyTorch]:
    """
    ...
    model_name, model_path = _download_model_PyTorch(model_name, download_root)
    tokenizer_path = _download_tokenizer_PyTorch(model_name, download_root)
    ...
    if "ssl" in model_name:
        model = GigaAM_PyTorch(checkpoint["cfg"])
    else:
        model = GigaAMASR_PyTorch(checkpoint["cfg"])
