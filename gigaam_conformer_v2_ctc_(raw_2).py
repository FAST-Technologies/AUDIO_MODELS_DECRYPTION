...
# os.environ["PATH"] += os.pathsep + "C:\Program Files (x86)\eSpeak\command_line"  # Adjust to your ffmpeg path
# print(os.environ["PATH"])
# venv_path = os.path.abspath(os.path.join(os.path.dirname(__file__), ".venv", "Scripts"))
# espeak_path = os.path.join(venv_path, "espeak.exe")
#
# if not os.path.exists(espeak_path):
#     print(
#         f"espeak.exe not found at {espeak_path}. Please ensure it is installed in .venv/Scripts.")
...
print("Type of log_probs_PyMONO:", type(log_probs_PyMONO))
print("Shape of log_probs_PyMONO:", log_probs_PyMONO.shape if isinstance(log_probs_PyMONO, np.ndarray) else "Not a numpy array")

# Проверка на nan и inf
if np.any(np.isnan(log_probs_PyMONO)):
    raise ValueError("log_probs_PyMONO contains NaN values")
if np.any(np.isinf(log_probs_PyMONO)):
    raise ValueError("log_probs_PyMONO contains Inf values")
...
if len(log_probs_PyMONO.shape) == 2:  # [seq_len, num_classes]
    ...
    raise ValueError(f"Unexpected shape for log_probs_PyMONO: {log_probs_PyMONO.shape}")
