...
from GigaAM_to_ONNX.CTC_model import CTCHead, CTCGreedyDecoding
from GigaAM_to_ONNX.FeatureExtractor import FeatureExtractor
from GigaAM_to_ONNX.GigaAM import GigaAM
from GigaAM_to_ONNX.GigaAMASR import GigaAMASR
from GigaAM_to_ONNX.Tokenizer import Tokenizer
from GigaAM_to_ONNX.SpecScaler import SpecScaler
from GigaAM_to_ONNX.LoadClass import _download_file, _download_model, _download_tokenizer, load_model
from GigaAM_to_ONNX.HelpFunction import load_audio, onnx_converter, print_statistic_data, plotter

# Импорт модулей для проверки работы нейронной сети на PyTorch
from CTC_v2_PyTorch.CTC_model_PyTorch import CTCHead_PyTorch, CTCGreedyDecoding_PyTorch
from CTC_v2_PyTorch.FeatureExtractor_PyTorch import FeatureExtractor_PyTorch
from CTC_v2_PyTorch.GigaAM_PyTorch import GigaAM_PyTorch
from CTC_v2_PyTorch.GigaAMASR_PyTorch import GigaAMASR_PyTorch
from CTC_v2_PyTorch.Tokenizer_PyTorch import Tokenizer_PyTorch
from CTC_v2_PyTorch.SpecScaler_PyTorch import SpecScaler_PyTorch
from CTC_v2_PyTorch.LoadClass_PyTorch import _download_file_PyTorch, _download_model_PyTorch, _download_tokenizer_PyTorch, load_model_PyTorch
from CTC_v2_PyTorch.HelpFunction_PyTorch import load_audio_PyTorch, onnx_converter_PyTorch, print_statistic_data_PyTorch, create_linear_filters_PyTorch, compute_lfcc_PyTorch
...
from CTC_v2_NumPy.CTC_model_NumPy import CTCHead_V2_0, CTCGreedyDecoding_V2_0
from CTC_v2_NumPy.FeatureExtractor_NumPy import FeatureExtractor_V2_0
from CTC_v2_NumPy.GigaAM_NumPy import GigaAM_V2_0
from CTC_v2_NumPy.GigaAMASR_NumPy import GigaAMASR_V2_0
from CTC_v2_NumPy.Tokenizer_NumPy import Tokenizer_V2_0
from CTC_v2_NumPy.SpecScaler_NumPy import SpecScaler_V2_0
from CTC_v2_NumPy.LoadClass_NumPy import _download_file_V2_0, _download_model_V2_0, _download_tokenizer_V2_0, load_model_V2_0
from CTC_v2_NumPy.HelpFunction_NumPy import load_audio_new_V2_0, load_audio_prev, print_statistic_data_V2_0, create_linear_filters_V2_0, compute_lfcc_V2_0
from CTC_v2_NumPy.GraphicsModule_NumPy import NumpyGraphicsModule

from HelpFunctions import tensor_info, decode_ctc_greedy

from Constants import Constants, VOCAB, _MODEL_NAMES
...
source_path = "E:/Прога (вся)/NeuralSpecter/AUDIO_MODELS_DECRYPTION/audio_files/-ot-vinta-bass-bossted.wav"
...
sessionONNX = rt.InferenceSession(f"{MY_CONSTANTS.DIRNAME}/{MY_CONSTANTS.MODEL_NAME}",
                              providers=["CPUExecutionProvider"])

try:
    tensor_info(flag="i", tensors=sessionONNX.get_inputs())
    tensor_info(flag="o", tensors=sessionONNX.get_outputs())
except Exception as e:
    print(f"Error retrieving model tensor information: {e}")

# Подготовка входных данных (для ONNX)
audioONNX = load_audio(audio_path=source_path)
print("Audio shape:", audioONNX.shape)
preprocessorONNX = FeatureExtractor(sample_rate=MY_CONSTANTS.SAMPLE_RATE,
                                    features=MY_CONSTANTS.FEAT_IN)

featuresONNX, lengthsONNX = preprocessorONNX(audioONNX.unsqueeze(0), torch.tensor([audioONNX.shape[-1]]))
featuresONNX = featuresONNX.detach().cpu().numpy().astype(np.float32)
lengthsONNX = lengthsONNX.detach().cpu().numpy().astype(np.int64)

# Вывод статистики
print_statistic_data(featuresONNX)

# Для примера: отображение первой фичи в батче (если features.shape = [1, 64, T])
plotter(featuresONNX)

# Инференс (для ONNX)
inputsONNX = {"features": featuresONNX, "feature_lengths": lengthsONNX}
log_probsONNX = sessionONNX.run(["log_probs"], inputsONNX)[0]

# Декодирование (жадное)
transcriptionONNX = decode_ctc_greedy(
    log_probs=log_probsONNX,
    vocab=VOCAB,
    blank_idx=MY_CONSTANTS.BLANK_IDX,
    max_vocab_idx=max_vocab_idx
)
print("Транскрипция:", transcriptionONNX)
...
try:
    tensor_info(flag="i", tensors=sessionPyTorch.get_inputs())
    tensor_info(flag="o", tensors=sessionPyTorch.get_outputs())
except Exception as e:
    print(f"Error retrieving model tensor information: {e}")
...
audioMONO = load_audio_PyTorch(audio_path=source_path,
                               load_type="mono")
print("Audio shape:", audioMONO.shape)
...
PyTorchGraphicsModule.plot_waveform_PyTorch(waveform=audioMONO,
                                            ...
                                            title="Original waveform 1")
PyTorchGraphicsModule.plot_waveform_PyTorch(waveform=audioSTEREO,
                                            sr=MY_CONSTANTS.SAMPLE_RATE,
                                            title="Original waveform 2")
PyTorchGraphicsModule.plot_waveform_PyTorch(waveform=reconstructed_waveform,
                                            sr=MY_CONSTANTS.SAMPLE_RATE,
                                            title="Reconstructed waveform 1")
print("AUDIO: ")
Audio(audioMONO.numpy(), rate=MY_CONSTANTS.SAMPLE_RATE)
Audio(audioSTEREO.numpy(), rate=MY_CONSTANTS.SAMPLE_RATE)
...
print("Audio shape 2:", audioSTEREO.shape)
print(f"Диапазон значений итогового аудио: [{audioSTEREO.min()} ; {audioSTEREO.max()}]")

# Сравнение
print(f"Максимальное расхождение для первого примера (моно): {torch.max(torch.abs(audioONNX - audioMONO)).item()} единицы")
print(f"Максимальное расхождение для второго примера (стерео): {torch.max(torch.abs(audioONNX - audioSTEREO)).item()} единицы")
...
PyTorchGraphicsModule.plot_fbank_PyTorch(mel_filters=mel_filters_PyTorch,
                                         title="Mel Filter Bank - torchaudio 1")
featuresPyMONO, lengthsPyMONO = preprocessorPyTorch(audioMONO.unsqueeze(0), torch.tensor([audioMONO.shape[-1]]))
...
featuresPySTEREO, lengthsPySTEREO = preprocessorPyTorch(audioSTEREO.unsqueeze(0), torch.tensor([audioSTEREO.shape[-1]]))
...
# Декодирование (жадное)
transcription_PyMONO = decode_ctc_greedy(
    log_probs=log_probs_PyMONO,
    vocab=VOCAB,
    blank_idx=MY_CONSTANTS.BLANK_IDX,
    max_vocab_idx=max_vocab_idx
)
print("Транскрипция:", transcription_PyMONO)
...
# Загрузка ONNX-сессии
session_NumPy = rt.InferenceSession("./onnx_models/v2_ctc.onnx",
                                    providers=["CPUExecutionProvider"])

# Проверка входов и выходов модели
try:
    tensor_info(flag="i", tensors=session_NumPy.get_inputs())
    tensor_info(flag="o", tensors=session_NumPy.get_outputs())
except Exception as e:
    print(f"Error retrieving model tensor information: {e}")
...
audio_NumPy_STEREO = load_audio_new_V2_0(audio_path=source_path,
                                         load_type="stereo")
print("Audio shape (stereo):", audio_NumPy_STEREO.shape)

print("Audio shape 2:", audio_NumPy_STEREO.shape)
...
NumpyGraphicsModule.plot_fbank(mel_filters=mel_filters_NumPy,
                               title="Mel Filter Bank - Numpy 1")

features_NumPy_MONO, lengths_NumPy_MONO = preprocessor_NumPy(audio_NumPy_MONO[np.newaxis, :], np.array([audio_NumPy_MONO.shape[-1]], dtype = np.int64))
features_NumPy_MONO = features_NumPy_MONO.astype(np.float32)
lengths_NumPy_MONO = lengths_NumPy_MONO.astype(np.int64)

features_NumPy_STEREO, lengths_NumPy_STEREO = preprocessor_NumPy(audio_NumPy_STEREO[np.newaxis, :], np.array([audio_NumPy_STEREO.shape[-1]], dtype = np.int64))
...
# Декодирование (жадное)
transcription_NumPy = decode_ctc_greedy(
...
# Декодирование (Дальнейшая идея использования)
